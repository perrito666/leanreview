...
// inside the file, like an unbounded -U. Gap rows carry both line numbers but
// no Source: hosts anchor review comments to diff positions, so lines outside
// the hunks are deliberately not commentable. Hunk rows keep their exact
// Sources, so comments, threads, and navigation behave identically in both
// views.
//
