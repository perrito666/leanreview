...
And a second mapping for the config file:

- **Schema URL**:
  `https://perrito666.github.io/leanreview/schema/leanreview-config.schema.json`
- **File path pattern**: `*/leanreview/config.json`

You now get structure validation, enum completion (`side`/`state` in
exchanges; action names for `keys` remaps in the config), and field
documentation on hover. Configs generated by `leanreview --init-config`
also carry a `$schema` reference, which JetBrains IDEs honour directly.
